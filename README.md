...
```
 defp deps do
    [
      ...
      {:gettext, "~> 0.11"},
      {:cowboy, "~> 1.0"},
      {:materia_commerce, "~> 0.1.0"}, #<- add here
    ]
  end
```
...
```
# Configures Guardian
config :materia_commerce, materia_commerce.Authenticator,
...
```
# Configure materia_commerce repo
config :materia_commerce, :repo, YourApp.Repo  #<- add your app repo
```
...
do generate migration file for materia_commerce and migrate

```
> mix materia_commerce.gen.migration
> mix ecto.create
> mix ecto.migrate
```
...
```
  pipeline :guardian_auth do
    plug materia_commerce.AuthenticatePipeline #<-- guardian jwt token authentication by user model.
  end
  pipeline :grant_check do
    plug materia_commerce.Plug.GrantChecker, repo: YourApp.Repo #<-- Grant check by user ,role and grant model.
  end
```

add materia_commerce user and grant model path.

lib/your_app_web/router.ex

```
  scope "/your-path", materia_commerceWeb do
    pipe_through [ :api]
...
materia_commerce grant is white list about user role and request mothod.

materia_commerce.Plug.GrantChecker provide simple role check function.
...
```
alias materia_commerce.Accounts
...
※ grant.role "anybody" is a special reserved keyword by materia_commerce and its means all roles.
※ grant.method "ANY" is a special reserved keyword by materia_commerce and its means all request methods.
...
to
```
{:materia_commerce, git: "https://bitbucket.org/karabinertech_bi/materia_commerce.git"},
```
...
#### Step2 replace Code
 ...
 code replace that
  ...
  - Servicex -> materia_commerce
  - servicex -> materia_commerce
  - materia_commerceErrorError -> BusinessError
...
```
> mix deps.clean servicex materia_commerce servicex_utils materia_commerce_utils
> mix deps.update materia_commerce materia_commerce_utils
> mix materia_commerce.gen.migration

```

If a foreign key construct to the servicex schema was defined,
You need to change the name of materia_commerce's migration file to be before the name of your migration file.

```
ls -1 priv/repo/migrations
20181006090000_create_your_table
20181206081940_materia_commerce_1_craete_users.exs
20181206081941_materia_commerce_2_craete_organizations.exs
20181206081942_materia_commerce_3_craete_address.exs
20181206081943_materia_commerce_4_craete_grants.exs
20181206081944_materia_commerce_5_craete_mail_templates.exs
```
...
```
ls -1 priv/repo/migrations
20180106081940_materia_commerce_1_craete_users.exs
20180106081941_materia_commerce_2_craete_organizations.exs
20180106081942_materia_commerce_3_craete_address.exs
20180106081943_materia_commerce_4_craete_grants.exs
20180106081944_materia_commerce_5_craete_mail_templates.exs
20181006090000_create_your_table
```
...
## Servicex -> materia_commerce change Over View  
 
#### move ServicexMatching.Accounts.User -> materia_commerce.Accounts.User
...
####  materia_commerce.Accounts.User
...
#### Mix.Tasks.materia_commerce.Gen.Migration 

  change do'nt execute Mix.Tasks.Guardian.Db.Gen.Migration.run([])
 
#### Servicex.Accounts.Address　-> materia_commerce.Locations.Address
...
#### materia_commerce.Authenticator.sign_in()
