defmodule MateriaCommerceWeb.RequestAppendixControllerTest do
  ...
  describe "update request_appendix" do
    ...
    test "renders request_appendix when data is valid", %{
      conn: conn,
      request_appendix: %RequestAppendix{id: id} = request_appendix
    } do
  ...
  describe "current request_appendix" do
    test "search", %{conn: conn} do
      conn =
        post(conn, request_appendix_path(conn, :search_current_request_appendices), %{
          "and" => [%{"request_number" => "PJ-01"}],
          "or" => []
        })

      json_response(conn, 200)
      |> Enum.map(fn request_appendix ->
        cond do
    ...
    test "new history", %{conn: conn} do
      create_conn =
        post(conn, request_appendix_path(conn, :current_request_appendices), %{
