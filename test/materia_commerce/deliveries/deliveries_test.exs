defmodule MateriaCommerce.DeliveriesTest do
  ...
  describe "deliveries" do
    ...
    test "list_deliveries/0 returns all deliveries" do
      delivery = delivery_fixture() |> @repo.preload([:snd_user, :rcv_user, :clt_user, :inserted, :updated])
    ...
    test "create_delivery/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Deliveries.create_delivery(%{}, @invalid_attrs, 1)
    end

    test "update_delivery/2 with valid data updates the delivery" do
      delivery = delivery_fixture()
      assert {:ok, delivery} = Deliveries.update_delivery(%{}, delivery, @update_attrs, 1)
    ...
    test "delete_delivery/1 deletes the delivery" do
      delivery = delivery_fixture()
      assert {:ok, %Delivery{}} = Deliveries.delete_delivery(%{}, delivery, 1)
